...
%              x_next2 = linspace(0, 0 - step_size2, ocp_N + 1);
...
save('ground_truth_trajectory.mat', 'x_star', 'u_star');
...
rectangle('Position', [107.5 -15 7.5 12.5], 'FaceColor', [0, 1, 0, 0.5]);
    ...
    % Set up the axis limits based on the initial position and range of motion
    xlim([min(min(x_sim(1,:)), min(x_sim(6,:)))-5, max(max(x_sim(1,:)), max(x_sim(6,:)))+5 ]);
    ylim([min(min(x_sim(2,:)), min(x_sim(7,:)))-10 , max(max(x_sim(2,:)), max(x_sim(7,:)))+10 ]);
%     axis equal;
%     
    % Animation loop
    for k = 2:length(x_sim)
