...
function model = DYNFourcarExternalCoupling()
...
W_x1 = [1e1,1e1,1e-4,5,1e-4]; %Car from down to up 
W_x2 = [1e1,1e1,1e-4,5,1e-4]; % Car from up to down
W_x3 = [1e1,1e1,1e-4,5,1e-4]; % Car from left to right
W_x4 = [1e1,1e1,1e-4,5,1e-4]; % Car from right to left
W_x = diag([W_x1,W_x2,W_x3,W_x4]);


% W_u = diag([1e0,0.50,1e0,0.50,1e-1,1e0,1e-1,1e0]);
% W_u = diag([1e-1,1e-3,1e0,1e-3,1e-1,1e0,1e-1,1e0]);
W_u = diag([0.02,0.008,0.02,0.008,1e-1,1e0,1e-1,1e0]);


W_xe1=[1e1,1e1,1e-2,1e1,1e-2];
W_xe2=[1e1,1e1,1e-5,1e1,1e-5];
W_xe3=[1e1,1e1,1e-2,1e1,1e-2];
W_xe4=[1e1,1e1,1e-5,1e1,1e-5];
