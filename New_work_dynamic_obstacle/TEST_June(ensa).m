...
W(1,1) = 1e1; W(2,2) = 1e-10; W(3,3) = 1e4 ;W(4,4) = 1e2; W(5,5)= 1e2; W(6,6)= 1e0; W(7,7)= 1e-10;
W_e = eye(5);
W_e(1,1) = 1e5; W_e(2,2)= 1e2; W_e(3,3)=1e2; W_e(4,4)=1e-2; W_e(5,5)=1e-10;
yr = zeros(ny, 1);
...
global dsafe
alpha = 5.0; dsafe= 2.5;
dist_squared = ((x(1) - x_obs)^2) + ((x(2))^2) - dsafe;
h =dist_squared;
a1 = x(1) - x_obs(1); a2 = v*cos(x(3)) - v_obs ; b1 = x(2); b2 = v*sin(x(3));
dh_dt = 2*((a1*a2) + (b1*b2));
...
C_val_monitor=[];
COST=[]; dt=0.02;
p_ref = zeros(1,ocp_N); p_ref(1)=200;
v_obs= 0.5;
for yy= 2:ocp_N
...
% y_points = [-20, -17, -13, -10];
%     x_points = [110, 107.5, 105, 102.5];
% y_points = [-20, -15, -12, -10];

%     x_points = [110, 107.5, 105, 102.5];
% y_points = [-20, -12.5, -9, -7.5];
% % Generate more points for the spline
% t = linspace(0, 1, ocp_N+1); % Parameter for the spline
% % Spline fitting
...
% 
    oo = 0;
%     theta_spline=linspace(pi/2,pi,ocp_N+1);
% theta_spline = linspace(pi/2,pi,ocp_N+1)
% % theta_spline = [theta_spline,pi/2];
...
% 
% % Generate more points for the spline
% t = linspace(0, 1, ocp_N-9); % Parameter for the spline
% 
% % Spline fitting
% pp_x = spline([0, 1/3, 2/3, 1], x_points); % Parametric x-coordinates
% pp_y = spline([0, 1/3, 2/3, 1], y_points); % Parametric y-coordinates
...
% x_horizontal = linspace(x_spline(end), 100, 10); % 10 points for the horizontal part
% y_horizontal = y_spline(end) * ones(1, 10); % y value remains constant
% 
...
ocp_N=100;
t = linspace(0, 1, ocp_N); % Parameter for the spline
...
x_spline = [x_spline, x_horizontal];
y_spline = [y_spline, y_horizontal];
theta_ref = atan2(diff(y_spline), diff(x_spline)); % Approximate heading
theta_ref = [theta_ref, theta_ref(end)]; % Extend the last heading
theta_spline=theta_ref;
...
for ii= 1 : N_sim
if x_sim(2,ii)<-20
             step_size=T*x_sim(4,ii);
             x_next = linspace(110, 110, ocp_N + 1);
             y_next = linspace(x_sim(2,ii), x_sim(2, ii) + step_size , ocp_N + 1);
           x_traj_init = [x_next; y_next; (pi/2)*ones(1, ocp_N + 1); 5 * ones(1, ocp_N + 1); zeros(1, ocp_N + 1)];
elseif x_sim(2,ii)>=-20 && x_sim(2,ii)<-7.5
if oo==0
x_traj_init = [x_spline; y_spline; theta_spline; ones(1,ocp_N+1); zeros(1,ocp_N+1)];
oo=oo+1;
else
x_traj_init = [x_traj_init(:,2:end), x_traj_init(:,end)];
end

elseif x_sim(2,ii)>= -7.5
%      step_size=T*x_sim(4,ii);
     step_size=T*1;
     x_next = linspace(x_sim(1,ii), x_sim(1,ii) - step_size, ocp_N + 1);
     y_next = linspace(x_sim(2,ii), x_sim(2, ii), ocp_N + 1);
     x_traj_init = [x_next; y_next; pi*ones(1,ocp_N+1); 1 * ones(1, ocp_N + 1); zeros(1, ocp_N + 1)];
end
...
radius = dsafe;
hold on
plot(Obstaclecenter,zeros(length(Obstaclecenter)),'bo')
road_width = 20;
road_length = 200;
...
if 1
    ...
    obstacle_circle = plot(Obstaclecenter(1), 0, 'bo');
    ...
    for k = 2:length(x_sim)
        ...
        set(obstacle_circle, 'XData', Obstaclecenter(k), 'YData', 0);
