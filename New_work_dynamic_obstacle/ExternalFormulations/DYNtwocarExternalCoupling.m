...
Gamma1 = -1.5;  % Adjust this parameter based on your risk preference
Gamma2 = +1.5;  % Adjust this parameter based on your risk preference

W_x = diag([1e3,1e3,1e-2,1e-2,1e-5,1e3,1e1,1e-2,1e-2,1e-10]);
W_u=diag([5,1e-1,1e-1,1e-1]);
...
% % Alternative 2 Softened Distance Penalty
% c = 1e-1;
% epsilon = 1e-2;
% coupling_term = c * ((x1 - x2)^2 + (y1 - y2)^2 + epsilon);

% % Alternative 3: Exponential Penalty (This formulation can create a sharper penalty as the distance increases.)
% c = 1e-3;
% coupling_term = c * exp((x1 - x2)^2 + (y1 - y2)^2);
% 
% % Alternative 4: Proximity-Based Penalty
% c = 1e-1;
% threshold = 5;
% distance_squared = (x1 - x2)^2 + (y1 - y2)^2;
% coupling_term = c * (distance_squared < threshold) * distance_squared;
% 
% Alternative 5: Angular Coupling
% c = 1e-1;
% beta = 1e-1;
% coupling_term = c * ((x1 - x2)^2 + (y1 - y2)^2) + beta * (theta1 - theta2)^2;
...
% expr_ext_cost = expr_ext_cost1 + expr_ext_cost2;
% 
% expr_ext_cost_e1 = (1 / Gamma1) * log(exp(Gamma1 * terminal_cost1));
% expr_ext_cost_e2 = (1 / Gamma2) * log(exp(Gamma2 * terminal_cost2));
% expr_ext_cost_e = expr_ext_cost_e1 + expr_ext_cost_e2;
%%
