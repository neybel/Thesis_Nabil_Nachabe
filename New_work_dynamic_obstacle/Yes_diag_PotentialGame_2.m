...
% constraints
x0 = [0; 10; -pi/4;10;0;0;-10;pi/4;10;0];
...
yr_e_new = [x0(1)+x0(4)*cos(x0(3))*T;x0(1)+x0(4)*sin(x0(3))*T;-pi/4;10;0;x0(6)+x0(9)*cos(x0(8))*T;x0(6)+x0(9)*sin(x0(8))*T;pi/4;10;0];
...
% x0(1)+x0(4)*cos(x0(3))*T
             ...
             step_size2=T*x_sim(9,ii)*cos(x_sim(8));
             step_size3=T*x_sim(4,ii)*sin(x_sim(3));
             step_size4=T*x_sim(9,ii)*sin(x_sim(8));
           ...
           x_traj_init = [x_next1; y_next1; (-pi/4)*ones(1, ocp_N + 1); 10 * ones(1, ocp_N + 1); zeros(1, ocp_N + 1);x_next2; y_next2; (pi/4)*ones(1, ocp_N + 1); 10 * ones(1, ocp_N + 1); zeros(1, ocp_N + 1)];
...
text(start_pos_agent1(1), start_pos_agent1(2) + 5, 'Agent 1 Start', 'Color', 'g', 'FontSize', 10, 'HorizontalAlignment', 'center');
text(start_pos_agent2(1), start_pos_agent2(2) + 5, 'Agent 2 Start', 'Color', 'g', 'FontSize', 10, 'HorizontalAlignment', 'center');


plot(x_sim(1,1),x_sim(2,1),'bo');
hold on

plot(x_sim(1,:),x_sim(2,:),'r*',x_sim(6,:),x_sim(7,:),'b','LineWidth',3);
legend('Agent 1 (Blue)', 'Agent 2 (Red)', 'Location', 'Best');  % Specify the legend labels directly
axis equal
...
%     axis equal;
    ...
    for k = 2:length(x_sim)
        ...
        % Pause to create animation effect
        pause(0.075); % Adjust the pause time to control the speed of the animation
    end
end
