...
% offset_y_agent2 = 10;  % Slight vertical offset for Agent 2
% 
% for i = time_steps
%     % Car for Agent 1
...
% Plot the road as a rectangle (centered at y=0)
rectangle('Position', [-10, -road_width/2, road_length, road_width], 'FaceColor', [0.7 0.7 0.7], 'EdgeColor', 'none', 'HandleVisibility', 'off');
hold on;

% Plot lane markings at the center, top, and bottom of the road
plot([-10 road_length], [-road_width/4, -road_width/4], 'w--', 'LineWidth', 2, 'HandleVisibility', 'off');  % Bottom lane marking
plot([-10 road_length], [road_width/4, road_width/4], 'w--', 'LineWidth', 2, 'HandleVisibility', 'off');    % Top lane marking

% Plot agent trajectories with transparency (alpha)
h1 = plot(x_sim(1,:), x_sim(2,:), 'r-', 'LineWidth', 6, 'Color', [1, 0, 0, 0.6], 'HandleVisibility', 'on');  % Agent 1 (Red)
h2 = plot(x_sim(6,:), x_sim(7,:), 'g-', 'LineWidth', 6, 'Color', [0, 1, 0, 0.6], 'HandleVisibility', 'on');  % Agent 2 (Green)
...
% Offset to avoid cars overlapping when they are close
offset_y_agent2 = 0.5;  % Slight vertical offset for Agent 2
...
for i = time_steps
    ...
    % Add step labels for both agents
    text(x_sim(1,i), x_sim(2,i) -4, num2str(i), 'Color', 'r', 'FontSize', 10, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'right');
    text(x_sim(6,i), x_sim(7,i) + offset_y_agent2 + 2, num2str(i), 'Color', 'g', 'FontSize', 10, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
end
