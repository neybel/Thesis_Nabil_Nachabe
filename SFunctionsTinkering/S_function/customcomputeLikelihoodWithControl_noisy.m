...
% std_control = [0.05; 0.005];  % For (acceleration, steering rate)
    ...
    importance_state_weights = [1e3, 1e3, 1e-5, 1e-5, 1e-5]; % Importance-based weights for state variables
    importance_control_weights = [1e3, 1e3];                 % Importance-based weights for control inputs
...
% Likelihood calculation using an adaptive scaling factor
    ...
    dynamic_constant = 5.0 * max(std_error2, std_error1);
