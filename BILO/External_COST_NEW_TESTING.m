...
model = kinematic_single_track_model_external();
...
%% NEW LIne (OLD Method)
% ocp_model.set('param_y_ref', x_ref);
%%
...
p_ref=zeros(3,ocp_N+1);
a=linspace(0,10,ocp_N+1);b=linspace(0,10,ocp_N+1);c=b;
p_ref(1,:)=a;p_ref(2,:)=b;p_ref(3,:)=0;
...
% 
% for k = 0:ocp_N-1
%     ocp.set(['cost_y_ref', num2str(k)], x_traj_init(:, k+1));
% end
% ocp.set('cost_y_ref_e', x_traj_init(:, end)); % Terminal cost reference
