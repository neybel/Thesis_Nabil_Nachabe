...
% 
W(1,1) = 1; W(2,2) = 1; W(3,3) = 1e-8; W(4,4) = 1e-3; W(5,5)=W(4,4);
W(6,6) = W(1,1); W(7,7) = W(2,2); W(8,8) = W(3,3); W(9,9) = W(4,4) ; W(10,10)=W(9,9);

% W_e = eye(6)*1e-2;
% W_e(1,1) = 1e10; W_e(2,2)= 10; W_e(4,4) = 1e7; W_e(5,5) = W_e(4,4);
% W(1,1) = 0.1; W(2,2) = 0.1; W(3,3) = 1e-8; W(4,4) = 1e-3; W(5,5)=W(4,4);
% W(6,6) = W(1,1); W(7,7) = W(2,2); W(8,8) = W(3,3); W(9,9) = W(4,4) ; W(10,10)=W(9,9);

W_e = eye(6)*1e-3;
W_e(1,1) = 10; W_e(2,2)=10; W_e(4,4) = W_e(1,1); W_e(5,5)=W_e(2,2);
% W_e(1,1) = 10; W_e(2,2)= 10; W_e(4,4) = 10; W_e(5,5)=10;
...
% dist_squared = sqrt((x1(1) - x2(1))^2 + (x1(2) - x2(2))^2); % ^2
% 
% % Define the barrier function parameters
% d_safe = 0.05;  % Safe distance threshold
% k_barrier = 1e4; % Barrier function gain
% 
% % Define the sigmoidal barrier function
% h_barrier = 1 / (1 + exp(k_barrier * (-dist_squared + d_safe))); % it was + before 
% 
...
%     ocp_model.set('cost_zl', 0.005 * ones(nsh,1));
%     ocp_model.set('cost_zu', 0* ones(nsh,1));
%     ocp_model.set('cost_Zl', 0.005 * ones(nsh,nsh));
...
x_traj_init1=[linspace(0,1,ocp_N+1);linspace(0,0,ocp_N+1);(pi/4)*zeros(1,ocp_N+1)];
% % x_traj_init=[x_traj_init;zeros(2,ocp_N+1)];
x_traj_init2=[linspace(1,0,ocp_N+1);linspace(0,0,ocp_N+1);-(pi/4)*zeros(1,ocp_N+1)];
...
x_sim(:,ii+1) = sim.get('xn');
    ...
    v_rel_longitudinal_test = u_curr(1) - u_curr(3);
    dh_dt_test = 2 * v_rel_longitudinal_test * (x_curr(1) - x_curr(4));
    h_test = dist_test;
...
%%
if 1
plot(x_sim(1,:),x_sim(2,:),'r*-'); hold on; plot(x_sim(4,:),x_sim(5,:),'k*-'); legend('car 1','car 2'); title('Trajectory XY'); xlabel('x'); ylabel('y');
axis equal
%Distance evaluation
