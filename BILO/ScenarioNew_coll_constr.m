...
W(1,1) = 5e4; W(2,2) = 5e4; W(3,3) = 1e-2; W(4,4) = 5e4; W(5,5)= 10;
W(6,6) = 100; W(7,7) = 100; W(8,8) = 1e-2; W(9,9) = 10 ; W(10,10)= 10;
...
W_e = eye(6)*1e-2;
W_e(1,1) = 5e6; W_e(2,2)=W_e(1,1); W_e(4,4) = 100; W_e(5,5) = W_e(4,4);
...
k_barrier = 1e-5; % Barrier function gain

% Define the sigmoidal barrier function
h_barrier = 1 / (1 + exp(k_barrier * (dist_squared - d_safe)));
...
ocp_model.set('cost_zl', 1e-1 * ones(nsh,1));
ocp_model.set('cost_zu', 0* ones(nsh,1));
% L2 slack (squared term)
ocp_model.set('cost_Zl', 1e-1 * ones(nsh,nsh));
