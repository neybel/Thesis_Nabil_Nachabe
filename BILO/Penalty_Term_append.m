...
% W(6,6) = 100; W(7,7) = 100; W(8,8) = 1e-2; W(9,9) = 10 ; W(10,10)=W(9,9);
...
W_e = eye(6)*1e-3;
W_e(1,1) = 50; W_e(2,2)=750; W_e(4,4) = W_e(1,1); W_e(5,5)=W_e(2,2);
...
lbu = [0;-pi;0;-pi];
ubu = [1;+pi;1;+pi];
%% Collisopn Avoidance Penalty
% Define symbolic variables for the states of both cars
x1 = model.sym_x(1:3);
x2 = model.sym_x(4:6);
% Define symbolic variable for squared distance
dist_squared = (x1(1) - x2(1))^2 + (x1(2) - x2(2))^2;

% Define weighting matrix for collision avoidance cost
W_collision = 1250*eye(6); % Adjust as needed

% Define collision avoidance cost term
collision_cost =  + dist_squared * W_collision;
%% Append this penalty term
% W_augmented = blkdiag(W, W_collision);
% W_e_augmented = blkdiag(W_e, zeros(size(W_collision)));
% Vx_augmented = blkdiag(Vx, zeros(size(W_collision))); % Add zeros for control variables
% Vx_e_augmented = blkdiag(Vx_e, zeros(size(W_collision)));
% Vu_augmented = blkdiag(Vu, zeros(size(W_collision, 1), nu));
% yr_augmented = [yr; zeros(size(W_collision, 1), 1)];
% yr_e_augmented = [yr_e; zeros(size(W_collision, 1), 1)];
expr_ext_cost = Vu' * W * Vx;

% Define the external cost expression for the terminal cost
expr_ext_cost_e = Vx_e' * W_e * Vx_e;
...
if (strcmp(cost_type, 'linear_ls'))
    ocp_model.set('cost_Vu', Vu_augmented);
    ocp_model.set('cost_Vx', Vx_augmented);
    ocp_model.set('cost_Vx_e', Vx_e_augmented);
    ocp_model.set('cost_W', W_augmented);
    ocp_model.set('cost_W_e', W_e_augmented);
    ocp_model.set('cost_y_ref', yr_augmented);
    ocp_model.set('cost_y_ref_e', yr_e_augmented);
    ocp_model.set('cost_expr_ext_cost', collision_cost);
