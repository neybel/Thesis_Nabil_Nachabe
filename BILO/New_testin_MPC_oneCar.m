...
% for ii=nu+1:nu+nx/2 W(ii,ii)=1e-1; end
% for ii=nu+floor(nx/2)+1:nu+nx W(ii,ii)=1e1; end
W(1,1) = 1e3; W(2,2) = W(1,1); W(3,3) = 1; W(4,4) = 1e-1; W(5,5)=W(4,4);
% W_e = W(nu+1:nu+nx, nu+1:nu+nx); % weight matrix in mayer term
W_e = eye(3)*1e-1;
W_e(1,1) = 1e2;
W_e(2,2)= 2e2;
...
x_traj_init=linspace(0,1,ocp_N+1);
x_traj_init=[x_traj_init;x_traj_init;(pi/4)*ones(1,ocp_N+1)];
...
for ii= 1 : N_sim
    ...
    for jj = 0 : ocp_N - 1
    yref = [x_traj_init(1,jj + 1);x_traj_init(2,jj+1);pi/4;0;0];
    ocp.set('cost_y_ref', yref, jj);
    end
    %consider to add ocp.set for the terminal stage as well, check the
    %example of minimal_example_closed loop. 
...
%             keyboard
	...
	x_traj_init = [x_traj(:,2:end), x_traj_init(:,end)];
	u_traj_init = [u_traj(:,2:end), u_traj(:,end)];
	u_sim(:,ii) = ocp.get('u', 0); %% check this line properlyy!!! Now it should be good
...
if DO_PLOT
    ...
    xlim([0 N_sim]);
    legend('x', 'y', 'theta');
        ylabel('states'); xlabel('N');
    subplot(3,1,2);
    plot(0:N_sim-1, u_sim);
    xlim([0 N_sim]);
    legend('F');
            ylabel('control inputs'); xlabel('N')
    subplot(3,1,3);
