...
W = eye(ny); 
W(1,1) = 1e-2; W(2,2) = 1e-2; W(3,3) = 1e2; W(4,4) = 1e2; W(5,5) = 1e-1; W(6,6) = 1e-1; W(7,7)=1e-1;
W_e = eye(5)*1e-1;
W_e(1,1) = 1e4; W_e(2,2)= 1e4; W_e(3,3) = 1e1; W_e(4,4)=1e-1; W_e(5,5) = 1e-1;
yr = zeros(ny, 1); yr_e = [100;100;0;0;0];
% Constraints
x0 = [0; 0; pi/4; 10; 0];Jbu = zeros(nbu, nu); for ii=1:nbu Jbu(ii,ii)=1.0; end
lbu = [-11.5;-0.4];
ubu = [11.5;+0.4];
%% acados ocp model
...
x_traj_init=[x_traj_init;x_traj_init;(pi/4)*ones(1,ocp_N+1);10*ones(1,ocp_N+1);zeros(1,ocp_N+1)];
...
%             keyboard
	...
	u_traj_init = [u_traj(:,2:end), u_traj(:,end)]; %check the shifting in an example they are taking the x directly from the ocp.get and not the inital trajectory as static like I did
