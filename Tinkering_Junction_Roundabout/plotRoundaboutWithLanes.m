function plotRoundaboutWithLanes()
    ...
    % Lane boundaries
    laneWidth = 10; % Increased lane width
    ...
    figure;
    hold on;
    fill(outerX, outerY, [0.7 0.7 0.7], 'EdgeColor', 'k','HandleVisibility', 'off'); % Outer circle
    fill(innerX, innerY, [0.3 0.3 0.3], 'EdgeColor', 'k','HandleVisibility', 'off'); % Inner circle (central island)
...
%     end
    ...
    for i = 1:numEntryExit
        ...
        for j = 0:1
            ...
            plot(ex, ey, 'k-', 'LineWidth', 2,'HandleVisibility', 'off');
        end
    ...
    axis equal;
    xlim([-outerRadius-entryExitLength+30, outerRadius+entryExitLength-30]);
    ylim([-outerRadius-entryExitLength+10, outerRadius+entryExitLength-10]);
