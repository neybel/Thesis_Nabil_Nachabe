...
if car
    W_xu1 = [10;10;1e-4;1e-4;1e-4;1;1];
end
...
std_dev=[2.5; 2.5; 1e-3; 1e-3; 1e-6; 3e-1;0.25];
...
% std_control = [0.001; 0.001];  % For (acceleration, steering rate)
X_obs1=[x0(1:5)];X_obs2=[x0(6:10)];
U_obs1=[];U_obs2=[];
x01=x0;
K=13; %it was 20 before
...
[particles1c,estimated_W_xuOfcar2,ess1,likelihoods1c] = customparticleFilter(X_obs2, U_obs2 ,x01, N, customnum_particles, W_xu_ranges,W_xu1,ocp,initial_guess_belief_car2,range_factor,car,std_dev,testtheta1,testtheta2,pf,D1,D2,K,k);
    N = size(X_star2, 2) - 1; 
pf=0; %it means the pf belongs to car 2
[particles2c,estimated_W_xu_Ofcar1,ess2,likelihoods2c] = customparticleFilter(X_obs1, U_obs1 ,x01, N, customnum_particles,W_xu_ranges, W_xu1,ocp,initial_guess_belief_car1,range_factor,car,std_dev,testtheta1,testtheta2,pf,D1,D2,K,k);
end
...
range_factor = max(0.05, range_factor * 0.9855); %Reduction strategy
...
% Create title string with ground truth values
title_str1 = sprintf('Evolution for Estimation for Weights of Car 1\nGround Truth: Wx1 = %.2e, Wx2 = %.2e', W_xu1(1), W_xu1(2));
title_str2 = sprintf('Evolution for Estimation for Weights of Car 1\nGround Truth: Wu1 = %.2e', W_xu1(6));
title_str3 = sprintf('Evolution for Estimation for Weights of Car 1\nGround Truth: Wu2 = %.2e', W_xu1(7));
...
fill([1:length(w_evol1) fliplr(1:length(w_evol1))], ...
 ...
 subplot(3,1,2)
 plot(1:length(w_evol1),w_evol1(6,:)); xlabel('Iteration'); ylabel('Magnitude');
 ...
 legend('Wu1')
 subplot(3,1,3)
 plot(1:length(w_evol1),w_evol1(7,:)); xlabel('Iteration'); ylabel('Magnitude');
...
title_str1 = sprintf('Evolution for Estimation for Weights of Car 2\nGround Truth: Wx1 = %.2e, Wx2 = %.2e', W_xu1(1), W_xu1(2));
title_str2 = sprintf('Evolution for Estimation for Weights of Car 2\nGround Truth: Wu1 = %.2e', W_xu1(6));
title_str3 = sprintf('Evolution for Estimation for Weights of Car 2\nGround Truth: Wu2 = %.2e', W_xu1(7));

 subplot(3,1,1)
 plot(1:length(w_evol1),w_evol2(1,:),1:length(w_evol1),w_evol2(2,:)); xlabel('Iteration'); ylabel('Magnitude');
...
fill([1:length(w_evol1) fliplr(1:length(w_evol1))], ...
 ...
 subplot(3,1,2)
 plot(1:length(w_evol1),w_evol2(6,:)); xlabel('Iteration'); ylabel('Magnitude');
 ...
 legend('Wu1')
 subplot(3,1,3)
 plot(1:length(w_evol1),w_evol2(7,:)); xlabel('Iteration'); ylabel('Magnitude');
