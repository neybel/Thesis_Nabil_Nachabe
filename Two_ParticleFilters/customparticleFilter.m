...
%     plot(X_sim(1,:),X_sim(2,:),'ko',LineWidth=5)
%     grid on;
%     grid minor; 
%     hold on
%     for u=1:num_particles
%     plot(predicted_trajectories{u}(1,:),predicted_trajectories{u}(2,:),'-',LineWidth=0.5);
%     hold on
%     end
%     legend('Observations','predicted trajectory')
%     xlabel('x [m]'); ylabel('y [m]'); title('Obeservations vs predicted trajectories')
