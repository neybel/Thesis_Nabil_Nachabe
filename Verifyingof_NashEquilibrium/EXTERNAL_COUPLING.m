...
yr_e_new = [110;x0(1)+x0(2)+x0(4)*T;pi/2;x0(4);0;x0(6)-x0(9)*T;0;pi;x0(9);0];
...
for ii= 1 : N_sim
    ...
    if closest_idx + 100 <= size(D, 2)
        reference_points = D(:, closest_idx:closest_idx+ocp_N);
    else
...
%         reference_points = D(:, end-100:end);
  % If there are not enough points, extend the trajectory
        ...
        additional_points = ocp_N - remaining_points;
...
grid on;
hold on;
plot(x_traj(1,:),x_traj(2,:),'r--',x_traj(6,:),x_traj(7,:),'b--');
