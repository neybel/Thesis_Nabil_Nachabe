...
%              x_next1 = linspace(x_sim(1, ii), x_sim(1, ii) + step_size1, ocp_N + 1);
             x_next2 = linspace(x_sim(1, ii), x_sim(1, ii) + step_size2, ocp_N + 1);
             x_next4 = linspace(x_sim(6, ii), x_sim(6, ii) - step_size2, ocp_N + 1);
%              y_next2 = linspace(-5,-5 + step_size4 , ocp_N + 1);
           x_traj_init = [x_next2; zeros(1,ocp_N+1); 0*ones(1, ocp_N + 1); x0(4) * ones(1, ocp_N + 1); zeros(1, ocp_N + 1);
               x_next4;  zeros(1,ocp_N+1);pi*ones(1,ocp_N+1);x0(9) * ones(1, ocp_N + 1);zeros(1, ocp_N + 1)];
...
hold on 
plot(x_traj(1,:),x_traj(2,:),'r*-',x_traj(6,:),x_traj(7,:),'bo');
axis equal
