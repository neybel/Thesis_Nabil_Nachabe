...
function model = DYNtwocarExternal_Perpendicular1()
...
Gamma1 = -0.1;  % Adjust this parameter based on your risk preference
...
%% Define stochastic cost expressions in here we are trying to add the noise on the states directly and not on the cost like done before!
state_cost = 0;
input_cost = 0;
...
for i = 1:2
    idx = (i-1)*5 + (1:5);
        ...
        % Add randomness to the state and input cost
            state_cost_sample = ((sym_x(idx)+normrnd(0, 0.001)) - p(idx))' * W_x(idx, idx) * ((sym_x(idx)+normrnd(0, 0.001)) - p(idx));
            input_cost_sample = (sym_u(2*i-1:2*i)+normrnd(0, 0.001))' * W_u(2*i-1:2*i, 2*i-1:2*i) * (sym_u(2*i-1:2*i)+normrnd(0, 0.001));
...
%     if any(Gamma)
%     state_cost = state_cost + abs((1 / Gamma(i))) * log(expected_value);
%     else
