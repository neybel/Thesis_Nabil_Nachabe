...
% model = DYNtwocarExternalCoupling();
% model = DYNtwocarExternal_Perpendicular11(); %noise on states
model = DYNtwocarExternal_Perpendicular11_new(); %noise on control input
h = 0.02;
...
nbx = 2; % number of bounds on state x
...
Jbx = zeros(2, nx); 
Jbx(1,idx_v1) = 1; % only constrain the velocity state
Jbx(2,idx_v2) = 1; % only constrain the velocity state
...
for ii= 1 : N_sim
    	ocp.set('constr_x0', x_sim(:,ii));
             ...
             x_traj_init = [zeros(1,ocp_N+1);y_next1;(pi/2)*ones(1,ocp_N+1) ;x0(4) * ones(1, ocp_N + 1); zeros(1, ocp_N + 1);x_next2; zeros(1,ocp_N+1); (0)*ones(1, ocp_N + 1); x0(9) * ones(1, ocp_N + 1); zeros(1, ocp_N + 1)];
...
if any(STATUS == 4)
    disp('there has been a status 4!!')
...
% ylim([min(x_sim(2,:)) - 5, max(x_sim(2,:)) + 10]);
axis equal
xlabel('x [m]');ylabel('y [m]'); title('Trajectory'); grid on; grid minor; hold on;
...
saveas(gcf, 'Cont1.pdf','pdf');

figure (300)
plot(1:N_sim,COST); title('Cost');grid on; grid minor;
grid on
