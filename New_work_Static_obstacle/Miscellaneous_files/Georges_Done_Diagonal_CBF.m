...
W(1,1) = 1e10; W(2,2) = 1e-10; W(3,3) = 1e2; W(4,4) = 1e-1; W(5,5)= 1e-1; W(6,6)= 1e-1; W(7,7)= 1e2;
W_e = eye(5);
W_e(1,1) = 1e2; W_e(2,2)= 1e2; W_e(3,3)=1e1; W_e(4,4)=1e-1; W_e(5,5)=1e-1;
yr = zeros(ny, 1);
yr_e = [100;100;0;0;0];
% constraints
x0 = [0; 0; pi/4;10;0];
...
yr_e_new = [x0(1)+x0(4)*cos(x0(3))*T;x0(1)+x0(4)*sin(x0(3))*T;pi/4;x0(4);0];
x_traj_init=[linspace(0,x0(1)+x0(4)*cos(x0(3))*T,ocp_N+1);linspace(0,x0(1)+x0(4)*sin(x0(3))*T,ocp_N+1);(pi/4)*ones(1,ocp_N+1);x0(4)*ones(1,ocp_N+1);zeros(1,ocp_N+1)];
...
% yr_e_new(2) = 0; 
yr_e_new(3) = pi/4;
test_traj_x = [x_traj(:,2),x_traj_init(:,3:end)];
x_traj_init = [test_traj_x, yr_e_new];
u_traj_init = [u_traj(:,2),u_traj_init(:,3:end),u_traj_init(:,end)];
%also I think it needs to be shifted properly
