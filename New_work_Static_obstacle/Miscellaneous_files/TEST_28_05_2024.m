...
W(1,1) = 1e10; W(2,2) = 1e-10; W(3,3) = 1e1; W(4,4) = 1e0; W(5,5)= 1e-5; W(6,6)= 1e-2; W(7,7)= 1e-7;
W_e = eye(5);
W_e(1,1) = 1e1; W_e(2,2)= 1e0; W_e(3,3)=1e-5; W_e(4,4)=1e-1; W_e(5,5)=1e-10;
yr = zeros(ny, 1);
yr_e = [10*T;0;0;10;0];
% constraints
x0 = [0; 0; 0;10;0];
...
yr_e_new(1) = x_traj_init(1,end) + x_traj(4,1) * 0.1;
u_sim(:,ii) = ocp.get('u', 0);
sim.set('x',x_sim(:,ii));
sim.set('u', u_sim(:,ii));
sim.solve();
x_sim(:,ii+1) = sim.get('xn');
x_traj_init = [linspace(x_sim(1,ii+1),x_traj_init(1,end) + x_traj(4,1)*0.1,ocp_N+1);linspace(0,0,ocp_N+1);zeros(1,ocp_N+1);10*ones(1,ocp_N+1);zeros(1,ocp_N+1)] ;
