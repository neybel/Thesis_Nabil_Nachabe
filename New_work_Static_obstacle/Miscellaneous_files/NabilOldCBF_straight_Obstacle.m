...
W(1,1) = 1e10; W(2,2) = 1e-10; W(3,3) = 1e3; W(4,4) = 1e-1; W(5,5)= 1e-1; W(6,6)= 1e-1; W(7,7)= 1e-10;
W_e = eye(5);
W_e(1,1) = 1e2; W_e(2,2)= 1e1; W_e(3,3)=1e-1; W_e(4,4)=1e-1; W_e(5,5)=1e-1;
yr = zeros(ny, 1);
yr_e = [100;0;0;0;0];
% constraints
x0 = [0; 0; 0;10;0];
...
% x = model.sym_x(1:3);
% dis = sqrt(((x(1) - obstacle_center(1))^2) + ((x(2) - obstacle_center(2))^2));
% d_safe = 0.25;  % Safe distance threshold
% k_barrier = 1e0; % Barrier function gain
% 
...
% C=h_barrier_with_margin;
% constr.expr_h = C;
%% Normal Distance constraint testing
% x = model.sym_x(1:3);
% dis = sqrt(((x(1) - obstacle_center(1))^2) + ((x(2) - obstacle_center(2))^2));
% threshold = 0.05; 
% h = dis - threshold; C=h; constr.expr_h = C;
%% New CBF 
% radius_car = 0.25; 
% radius_obs = 1.0;
...
global alpha 
global dsafe
alpha = 0.75; dsafe= 1.75;
dist_squared = ((x(1) - obstacle_center(1))^2) + ((x(2) - obstacle_center(2))^2) - dsafe;
% dist_squared = ((x(1) - obstacle_center(1))^2) + ((x(2) - obstacle_center(2))^2);
h =dist_squared;
...
% safe_distance = 5; % Safe distance to avoid the obstacle
    ...
    goal_x = 100;
goal_y = 0; Kp_steering = 10;  % Proportional gain for steering rate adjustment
dt=0.1;
    tic;
for ii= 1 : N_sim
...
%     if full(C_val) < 0
%         u_traj_init(2, 1) = 0.05;  % Set appropriate steering rate
%     else
%             % Calculate the desired heading to the goal
%     current_x = full(x_val(1));
%     current_y = full(x_val(2));
%     desired_heading = atan2(goal_y - current_y, goal_x - current_x);
%     
%     % Calculate the current heading
%     current_heading = full(x_val(3));
%     
%     % Calculate the heading error
%     heading_error = desired_heading - current_heading;
%         heading_error = atan2(sin(heading_error), cos(heading_error));
%         steering_rate_adjustment = Kp_steering * heading_error / dt;
%     % Adjust the steering rate proportionally to reduce the heading error
%         u_traj_init(2, 1) = full(steering_rate_adjustment);
%     end
%     % Ensure steering rate is within acceptable bounds (if necessary)
% max_steering_rate = 0.4;  % Define the maximum steering rate
% u_traj_init(2, 1) = max(min(u_traj_init(2, 1), max_steering_rate), -max_steering_rate);
    for jj = 0 : ocp_N - 1
%         if jj ~=ocp_N -1 
%         kk = jj + 1;
%         else
...
%         end
        kk = min(jj + 1, ocp_N-1);
%         distance_to_obstacle = norm(x_sim(1:2, ii)' - obstacle_position');

        % Dynamic steering rate reference
%         if distance_to_obstacle < safe_distance
%             % Calculate a simple steering angle to avoid the obstacle
%             desired_angle = atan2(obstacle_position(2) - x_sim(2, ii), obstacle_position(1) - x_sim(1, ii)) + pi/2;
%             u_traj_init(2, jj+1) = desired_angle; % Set steering rate reference
%         else
%             u_traj_init(2, jj+1) = 0; % No steering if far from the obstacle
%         end
...
yr_e_new(1) = x_sim(1,ii+1) + x_sim(4,ii+1)*T; 
yr_e_new(2) = 0; yr_e_new(3) = 0;
