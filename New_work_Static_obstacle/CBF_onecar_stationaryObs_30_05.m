...
W_e = eye(5);
W_e(1,1) = 1e1; W_e(2,2)= 1e1; W_e(3,3)=1e-5; W_e(4,4)=1e-1; W_e(5,5)=1e-10;
yr = zeros(ny, 1);
yr_e = [10*T;0;0;10;0];
% constraints
x0 = [0; 0; 0;10;0];
...
global alpha 
global dsafe
alpha = 5; dsafe= 2.5; %alpha equal to 1.0 it doesnt go much to the boudary, alpha = 5.0 it goes to the boundary. 
...
%             step_size = min(dist_to_goal, T * x_sim(4,ii)); 
             ...
             y_next = linspace(0, goal_y, ocp_N + 1);
           x_traj_init = [x_next; y_next; zeros(1, ocp_N + 1); 10 * ones(1, ocp_N + 1); zeros(1, ocp_N + 1)];
...
figure
plot(1:N_sim,C_val_monitor(1,1:end))
% set(gca, 'Color', [0.95, 0.95, 0.95]); % Light grey background
% set(gca, 'FontSize', 12, 'FontWeight', 'bold');
grid on 
box on 
title ('CBF Norm','Interpreter','latex')
xlabel('# iteration','Interpreter','latex');
ylabel('CBF value','Interpreter','latex');
% Compute the minimum value
min_CBF = min(C_val_monitor);
% Format the annotation text
annotationText = sprintf('Minimum CBF Value: %.2f', min_CBF);
annotation('textbox', [0.7, 0.8, 0.1, 0.1], 'String', annotationText, ...
           'FitBoxToText', 'on', 'BackgroundColor', 'white', ...
           'EdgeColor', 'black', 'FontSize', 12, 'FontWeight', 'bold');
%%
